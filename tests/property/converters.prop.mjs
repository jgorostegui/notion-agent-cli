...
const safeText = fc.stringOf(fc.char().filter(c => !/[#\-*>`~_\[\]()\\$\n\r]/.test(c)), { minLength: 1, maxLength: 200 })
  .map(s => s.trim())
  .filter(s => s.length > 0);
...
describe("Property 1: Markdown round-trip equivalence", () => {
  it("blocksToMarkdown(markdownToBlocks(md)) preserves block types, order, and text", () => {
    fc.assert(fc.property(
      fc.array(mdLine, { minLength: 1, maxLength: 20 }),
      (lines) => {
        const md = lines.join("\n");
        ...
        for (const block of blocks) {
          ...
          const supported = ["paragraph", "heading_1", "heading_2", "heading_3",
            "bulleted_list_item", "numbered_list_item", "to_do", "quote", "divider", "code"];
...
describe("Property 2: blocksToMarkdown non-empty output for all block types", () => {
  ...
  it("type-appropriate markers present", () => {
    fc.assert(fc.property(safeText, (text) => {
      ...
      // List markers
      assert.ok(blocksToMarkdown([{ type: "bulleted_list_item", bulleted_list_item: { rich_text: rt } }]).includes("- "));
      assert.ok(blocksToMarkdown([{ type: "numbered_list_item", numbered_list_item: { rich_text: rt } }]).includes("1. "));
      // Quote marker
...
describe("Property 5: recreateBlock strips read-only fields and limits children", () => {
  const readOnlyFields = ["id", "created_time", "last_edited_time", "created_by",
