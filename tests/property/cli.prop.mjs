...
import { ACTIONS, toCamelCase, NotionActions } from "../../scripts/actions.mjs";
...
describe("Property 20: CLI input mode detection", () => {
  ...
  it("toCamelCase converts snake_case action names", () => {
    fc.assert(fc.property(
      fc.constantFrom("get_page", "query_database", "export_page", "find_duplicates",
        "batch_set_properties", "suggest_reorganization", "add_database_entry"),
      (snakeCase) => {
...
describe("Property 21: CLI output and exit codes", () => {
  it("string result → direct output, exit 0", () => {
    fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 200 }), (result) => {
  ...
  it("{ success: false } → JSON output, exit 1", () => {
    fc.assert(fc.property(
      fc.record({ success: fc.constant(false), error: fc.string() }),
      (result) => {
        const json = JSON.stringify(result, null, 2);
...
describe("Property 22: Return value contract", () => {
  const writeActions = [
    "createPage", "updatePage", "appendBlocks", "insertBlocks", "setProperties",
    "addComment", "lockPage", "unlockPage", "createDatabase", "addDatabaseEntry",
  ];
  const structuralActions = [
    "moveBlocks", "movePage", "reorderBlocks", "deepCopy", "mergePages", "splitPage",
    "replaceSection", "flattenPage", "nestUnderHeadings", "duplicateStructure", "applyTemplate",
  ];
...
describe("Property 23: Position parameter format", () => {
  it("after-block position produces correct nested format, never bare string", () => {
    fc.assert(fc.property(fc.uuid(), (blockId) => {
...
describe("Property 28: JSON request file parameter mapping", () => {
  it("named parameters mapped to correct positional/options args via action registry", () => {
    fc.assert(fc.property(
      fc.constantFrom(...Object.keys(ACTIONS)),
      (actionName) => {
