...
import {
  RateLimiter, buildPropertyValue, normalizeId, safeName,
  extractTitle, extractDbTitle, extractPropertyValue, csvEscape,
...
describe("Property 8: buildPropertyValue produces correctly typed objects", () => {
  ...
  it("number type produces { number: N }", () => {
    fc.assert(fc.property(fc.integer(), (val) => {
  ...
  it("multi_select handles arrays and single values", () => {
    fc.assert(fc.property(fc.array(fc.string(), { minLength: 1, maxLength: 5 }), (vals) => {
  ...
  it("checkbox type produces { checkbox: boolean }", () => {
    fc.assert(fc.property(fc.boolean(), (val) => {
...
const safeText = fc.stringOf(fc.char().filter(c => !/[\n\r]/.test(c)), { minLength: 1, maxLength: 100 })
...
describe("Property 27: normalizeId round-trip", () => {
  ...
  it("32-char hex → hyphenated UUID format", () => {
    fc.assert(fc.property(hex32, (hex) => {
