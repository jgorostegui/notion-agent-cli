...
import { ACTION_CATALOG, ACTIONS, ACTION_ALIASES } from "../../scripts/actions.mjs";
...
describe("ACTION_CATALOG completeness", () => {
	for (const [name, meta] of Object.entries(ACTION_CATALOG)) {
		it(`${name} has all required fields`, () => {
			...
			assert.ok(typeof meta.summary === "string" && meta.summary.length > 0, `${name}: summary must be non-empty string`);
			...
			assert.ok(typeof meta.output === "object" && typeof meta.output.kind === "string", `${name}: output.kind required`);
			...
			assert.ok(typeof meta.skill.includeInQuickPatterns === "boolean", `${name}: skill.includeInQuickPatterns must be boolean`);
			assert.ok(typeof meta.skill.includeInDecisionTree === "boolean", `${name}: skill.includeInDecisionTree must be boolean`);
...
describe("ACTIONS is the derived projection of ACTION_CATALOG", () => {
	...
	for (const [name, meta] of Object.entries(ACTION_CATALOG)) {
		it(`${name} args projection matches`, () => {
			assert.deepEqual(ACTIONS[name].args, meta.args.map((a) => a.name));
		});
		it(`${name} options projection matches`, () => {
			assert.deepEqual(ACTIONS[name].options, meta.options.map((o) => o.name));
		});
	}
});
